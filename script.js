...
function addIdea(text, key) {
  ...
  var ideaItem = $("<div>").addClass("idea-item").attr("data-key", key).append(ideaText, deleteButton);
  $("#ideas-container").append(ideaItem);
}

// Replace the local storage-based saving and loading functions with Firebase
function loadIdeas() {
...
function saveIdeas() {
  $("#ideas-container .idea-item").each(function () {
    const key = $(this).data("key");
    const text = $(this).find(".idea-text").text();
    ideasRef.child(key).set(text);
  });
}

// To handle deletions:
function removeIdea(key) {
  $(`#ideas-container .idea-item[data-key="${key}"]`).remove();
}
...
$(document).ready(function() {
  ...
  var key = $(this).closest(".idea-item").attr("data-key");
  ideasRef.child(key).remove();
});
  ...
  $("#ideas-container").on("blur", ".idea-text", function() {
    ...
    saveIdeas(); // Save updated list to Firebase
  });

  // Remove a text item when the delete button is clicked
  $(document).on("click", ".delete-button", function() {
    const key = $(this).closest(".idea-item").data("key");
    ideasRef.child(key).remove();
